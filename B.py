...
class Student:  
    ...
    def add_data(self):
        if self.dep.get() == "Select Department" or self.student_id.get() == "" or self.student_name.get() == "":
            ...
            try:
                conn = mysql.connector.connect(host = "localhost", username = "root", password = "$/F%rdyUQmX-i7", database = "frs")
    ...
    def generate_dataset(self):
        if self.dep.get() == "Select Department" or self.student_id.get() == "" or self.student_name.get() == "":
            ...
            try:
                ...
                mycursor = conn.cursor()
                import hashlib
                id = int(hashlib.sha256(str(self.student_id.get()).encode('utf-8')).hexdigest(), 16) % (10**8)

                sql = """UPDATE student SET Name = %s, Department = %s, Course = %s, Semester = %s, Year = %s, Mobile = %s, Email = %s ,School = %s, Parent_Name = %s, DOB = %s, Address = %s WHERE StudentID = %s"""  # (name, address) VALUES (%s, %s)"
                mycursor.execute(sql, (
                    self.student_name.get(), self.dep.get(), self.course.get(), self.semester.get(),
                    self.year.get(), self.mobile.get(),
                    self.email.get(), self.school.get(), self.parent_name.get(), self.dob.get(), self.address.get(),
                    self.student_id.get()))
                ...
                while True:
                    ...
                    if cropped is not None:
                        ...
                        file_path = "Faces/user." + str(id) + "." + str(img_id) + ".jpg"
